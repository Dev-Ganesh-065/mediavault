...
| 26 | Tablet/mobile (≤ 920 px): the column layout let `.gridwrap` grow to the virtualised canvas's full height (a 6,000 px grid on a phone), so the grid stopped scrolling internally — the detail panel was pushed below the fold (and "disappeared" once more rows loaded), and the grid's scroll events never fired, so infinite scroll stalled with the panel open | `styles.css` | **fixed** — `min-height: 0` on `.gridwrap` inside the narrow-window media query; the grid scrolls internally again, the panel stays visible (max-height 45 vh), and load-more works with the panel open. Verified at 1440/768/390 px: layout, panel visibility, load-more and cached re-clicks all pass |
